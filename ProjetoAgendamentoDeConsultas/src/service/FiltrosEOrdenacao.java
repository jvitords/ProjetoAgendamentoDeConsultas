...
public class FiltrosEOrdenacao {
	...
	public static Consulta filtrarPeloCPF(int cpfDoPaciente) { // filtrar a consulta pelo cpf do paciente
		...
		List<Consulta> lista = Agenda.mostrarTodasAsConsultas();
		List<Consulta> listaFiltrada = lista.stream().filter(c -> c.getPaciente().getCpf() == cpfDoPaciente).collect(Collectors.toList()); 
		Consulta consultaFiltrada = listaFiltrada.get(0);
